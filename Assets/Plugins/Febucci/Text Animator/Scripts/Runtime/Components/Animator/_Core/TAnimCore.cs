...
namespace Febucci.UI.Core
{
    ...
    public abstract class TAnimCore : MonoBehaviour
    {
        ...
        void ConvertText(string textToParse, ShowTextMode showTextMode)
        {
            ...
            void PopulateCharacters(bool resetVisibility)
            {
                ...
                for (int i = 0; i < charactersCount; i++)
                {
                    ...
                    float CalculateRegionMaxDuration(AnimationRegion[] tags)
                    {
                        ...
                        //For each tag
                        foreach (AnimationRegion tag in tags)
                        {
                            //for each range
                            foreach (TagRange range in tag.ranges)
                            {
            ...
            {
                CharacterData c = characters[charIndex];
                c.isVisible = false;
                c.passedTime = 0;
                c.Hide();
            ...
            {
                foreach (AnimationRegion region in regions)
                {
                    foreach (TagRange range in region.ranges)
                    {
            ...
            void AddFallbackEffectsFor<T>(ref AnimationRegion[] currentEffects, VisibilityMode visibilityMode, Database<T> database, string[] defaultEffectsTags) where T : AnimationScriptableBase
            {
                ...
                System.Collections.Generic.List<DefaultRegion> defaultRegions = new System.Collections.Generic.List<DefaultRegion>();
                string[] tagWords;
                string tagName;
                foreach (string tag in defaultEffectsTags)
                {
                ...
                {
                    foreach (DefaultRegion element in defaultRegions)
                    {
            ...
            AnimationParser<AnimationScriptableBase> ruleBehavior = new AnimationParser<AnimationScriptableBase>(settings.behaviors.openingSymbol, '/', settings.behaviors.closingSymbol, VisibilityMode.Persistent, databaseBehaviors);
            AnimationParser<AnimationScriptableBase> ruleAppearance = new AnimationParser<AnimationScriptableBase>(settings.appearances.openingSymbol, '/', settings.appearances.closingSymbol, VisibilityMode.OnVisible, databaseAppearances);
            AnimationParser<AnimationScriptableBase> ruleDisappearance = new AnimationParser<AnimationScriptableBase>(settings.appearances.openingSymbol, '/', '#', settings.appearances.closingSymbol, VisibilityMode.OnHiding, databaseAppearances);
            ...
            //TODO optimize
            System.Collections.Generic.List<TagParserBase> parsers = new System.Collections.Generic.List<TagParserBase>()
            {
                ruleBehavior,
                ruleAppearance,
                ruleDisappearance,
            ...
            foreach (TagParserBase extraParser in GetExtraParsers())
            {
            ...
            foreach (AnimationRegion behavior in behaviors) behavior.animation.InitializeOnce();
            foreach (AnimationRegion appearance in appearances) appearance.animation.InitializeOnce();
            foreach (AnimationRegion disappearance in disappearances) disappearance.animation.InitializeOnce();
