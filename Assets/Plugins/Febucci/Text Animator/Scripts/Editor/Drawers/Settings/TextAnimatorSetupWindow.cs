...
namespace Febucci.UI
{
    public class TextAnimatorSetupWindow : EditorWindow
    {
        ...
        static void ShowWindow(bool onlyOnUpdate)
        {
            // already installed
            bool shouldUpdate = false;
            if (IsTextAnimatorInstalled(out string installationGUID))
            {
                string installationPath = AssetDatabase.GUIDToAssetPath(installationGUID);
                TextAnimatorInstallationData installationData = AssetDatabase.LoadAssetAtPath<TextAnimatorInstallationData>(installationPath);
            ...
            TextAnimatorSetupWindow window = (TextAnimatorSetupWindow)GetWindow(typeof(TextAnimatorSetupWindow), true,
        ...
        static TextAnimatorInstallationData GetOrCreateInstallationData()
        {
            ...
            TextAnimatorInstallationData data = _CreateScriptableAssetAtPath<TextAnimatorInstallationData>(path_defaultInstallation + "/Data",
                "InstallationData");
        ...
        public static void ResetToBuiltIn()
        {
            ...
            TextAnimatorSettings settings = GetOrCreateSettings(installationFolder);
            CreateDefaultDatabases(installationFolder,
                out AnimationsDatabase beh,
                out AnimationsDatabase app,
                out ActionDatabase act,
                out StyleSheetScriptable stylesheet);
            ...
            where DatabaseType : Database<ElementType> where ElementType : ScriptableObject, ITagProvider
        {
            ...
            DatabaseType database = _CreateScriptableAssetAtPath<DatabaseType>(databasePath, fileName);
            Type[] types = GetScriptableElementsFromAssembly<ElementType>();
            for (int i = 0; i < types.Length; i++)
            {
                if (TryCreatingDefaultTagScriptable(types[i], category, out ScriptableObject so))
                {
        ...
        {
            TagInfoAttribute attribute = type.GetCustomAttributes(typeof(TagInfoAttribute), true).FirstOrDefault() as TagInfoAttribute;
            if (attribute == null)
            {
            ...
            DefaultValueAttribute[] defaultValueAttributes =
            ...
            SerializedProperty tagID = serializedSo.FindProperty("tagID");

            if (defaultValueAttributes != null)
            {
                foreach (DefaultValueAttribute info in defaultValueAttributes)
                {
        ...
        {
            StyleSheetScriptable result = _CreateScriptableAssetAtPath<StyleSheetScriptable>(installationFolder + $"/Styles", fileName_stylesheet);
        ...
        static bool UpdateProject(TextAnimatorInstallationData installationData, Version oldVersion, bool performUpdate)
        {
            ...
            if (!string.IsNullOrEmpty(installationData.latestVersion))
            {
                ...
                if (oldVersion < new Version(2, 1, 0))
                {
                    ...
                    {
                        StyleSheetScriptable styleSheet = CreateStyleSheet(installationFolder);
