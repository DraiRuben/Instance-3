...
public class ImmediateModeMonitor : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        GUILayout.Space(4);
        using (EditorGUILayout.ScrollViewScope scroll = new EditorGUILayout.ScrollViewScope(scrollPos, false, false))
        {
            ...
            foreach (System.Collections.Generic.KeyValuePair<Camera, System.Collections.Generic.List<DrawCommand>> kvp in DrawCommand.cBuffersRendering)
            {
            ...
            using (new EditorGUI.IndentLevelScope())
            {
                foreach (System.Collections.Generic.KeyValuePair<RenderState, Material> kvp in IMMaterialPool.pool)
                {
