...
namespace Shapes
{
    ...
    internal abstract class MetaMpb : IDisposable
    {
        ...
        public bool PreAppendCheck(ShapeDrawState additionDrawState, Matrix4x4 mtx)
        {
            ...
            else if (instanceCount < UnityInfo.INSTANCES_MAX && drawState.CompatibleWith(additionDrawState))
            { // it already exists, but if it's full, we can't merge. also check compatibility, otherwise no merge pls
                append = true;
            }
