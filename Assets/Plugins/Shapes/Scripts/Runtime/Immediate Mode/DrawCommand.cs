...
namespace Shapes
{
    ...
    public class DrawCommand : System.IDisposable
    {
        ...
        public static void ClearAllCommands()
        {
            ...
            foreach (List<DrawCommand> drawCmds in cBuffersRendering.Values)
            {
        ...
        {
            List<KeyValuePair<Camera, List<DrawCommand>>> sadCameras = cBuffersRendering.Where(kvp => kvp.Key == null).ToList();
            foreach (KeyValuePair<Camera, List<DrawCommand>> kvp in sadCameras)
            {
