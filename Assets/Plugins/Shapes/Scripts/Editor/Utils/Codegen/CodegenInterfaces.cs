...
public static class CodegenInterfaces
{
    ...
    public static void Generate()
    {
        ...
        IOrderedEnumerable<TypeInfo> allShapes = asm.DefinedTypes.Where(
            type => type.IsAbstract == false && (type.BaseType == typeof(ShapeRenderer) || type.BaseType?.BaseType == typeof(ShapeRenderer))
        ).OrderBy(type => type.FullName);
        IEnumerable<TypeInfo> shapesWithInterfaces = allShapes.Where(sh => sh.ImplementedInterfaces.Contains(typeof(IDashable)) || sh.ImplementedInterfaces.Contains(typeof(IFillable)));

        // generate code
        code = new CodeWriter();
        using (code.MainScope(typeof(CodegenInterfaces), "UnityEngine"))
        {
            foreach (TypeInfo shape in shapesWithInterfaces)
            {
                code.Spacer();
