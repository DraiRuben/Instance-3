...
namespace Shapes
{
    ...
    public class ShapesImportState : ScriptableObject
    {
        ...
        static void MakeSureSampleMaterialsAreValid()
        {
            ...
            bool changed = false;
            if (ShapesIO.TryMakeAssetsEditable(UIAssets.Instance.sampleMaterials))
            { // ensures version control allows us to edit
                foreach (Material mat in UIAssets.Instance.sampleMaterials)
                {
                    if (mat == null)
